...
`)
        ...
        try {
            ...
            const postUp = serverConfig.post_up || 'iptables -A FORWARD -i wg0 -j ACCEPT; iptables -A FORWARD -o wg0 -j ACCEPT; iptables -t nat -A POSTROUTING -o eth+ -j MASQUERADE'
            const postDown = serverConfig.post_down || 'iptables -D FORWARD -i wg0 -j ACCEPT; iptables -D FORWARD -o wg0 -j ACCEPT; iptables -t nat -D POSTROUTING -o eth+ -j MASQUERADE'

            const wgConfigContent = `# Auto-generated by SecDash
[Interface]
Address = ${serverConfig.addresses[0]?.replace('/24', '.1') || '10.252.1.1'}
...
`
        ...
        // 4. Ensure NAT/Masquerade Rules (Runtime safety) - only if interface available
        if (interfaceAvailable) {
            try {
                ...
                // 2. Masquerade (NAT) on eth interface
